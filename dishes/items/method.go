...
func (m *Model) GetList(ctx context.Context, merchantID string, accountID string, urlParams *rest.UrlParams) ([]*Model, int64, error) {
	...
	if urlParams.HasFilter {
		...
		// 添加更多过滤器
		if urlParams.FilterCommon.From != "" {
			filterByFrom := bson.E{Key: "from", Value: urlParams.FilterCommon.From}
			filters = append(filters, filterByFrom)
		}

		// 添加更多过滤器
		if urlParams.FilterCommon.Gender != "" {
...
// Render 返回渲染对象
func (m SpecificationPrice) Render(name string, normal string, normalVIP string, takeOut string, takeOutVIP string) SpecificationPrice {
	m.Name = name

	normalInt, _ := strconv.Atoi(normal)
	m.Normal = float64(normalInt)

	normalVIPInt, _ := strconv.Atoi(normalVIP)
	m.NormalVIP = float64(normalVIPInt)

	takeOutInt, _ := strconv.Atoi(takeOut)
	m.TakeOut = float64(takeOutInt)

	takeOutVIPInt, _ := strconv.Atoi(takeOutVIP)
	m.TakeOutVIP = float64(takeOutVIPInt)
...
func (m EnablesSwitch) Render(isOpen, isOnShelves, isSupportSigns, isSupportNew,
	isSupportRecommend, isSupportDiscount, isSupportDecimal, isSupportWeighing,
	isSupportAutoJoin, isSupportSoldSeparately string) EnablesSwitch {

	m.IsOpen = "启用" == isOpen
	m.IsOnShelves = "是" == isOnShelves
	m.IsSupportSigns = "是" == isSupportSigns
	m.IsSupportNew = "是" == isSupportNew
	m.IsSupportRecommend = "是" == isSupportRecommend
	m.IsSupportDiscount = "是" == isSupportDiscount
	m.IsSupportDecimal = "是" == isSupportDecimal
	m.IsSupportWeighing = "是" == isSupportWeighing
	m.IsSupportAutoJoin = "是" == isSupportAutoJoin
	m.IsSupportSoldSeparately = "是" == isSupportSoldSeparately
	return m
}

// RenderSupportBusinessList 返回支持业务的列表
// 堂食+外卖+自提
func (m *Model) RenderSupportBusinessList(business string) []string {
	const sep = "+"
	return strings.Split(business, sep)
}
